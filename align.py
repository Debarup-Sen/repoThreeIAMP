...
tool = lit.radio(
    ...
    ('BLASTp (Basic Local Alignment Search against IAMP)', 'MUSCLE (Multiple Sequence Alignment)',
...
if 'BLASTp' in tool:
    ...
    with col1:
        query = lit.text_area('Enter your input protein sequence (in FASTA/multi-FASTA format/plain text sequence format/IAMP Acc. ID, e.g. IAMP1) here',
                          height=200).upper()
    query = query.replace(' ', '')
    with col2:
...
###  Blast should reject "IAMP1" && add multi acc id 
    ...
    if (query or file_query) and submit:
        ...
        if query and 'IAMP' in query and '>' not in query:
            check = [i for i in query.split('\n') if i!= '']
            for i in check:
                if '_' in i and i.isalnum():
                    lit.info('If you are trying to enter IAMP Acc. ID, please make sure it is in correct format (e.g. IAMP1)')
                    query = None
                    break

        elif 'IAMP' not in query and '>' not in query and any(char.isdigit() for char in query):
        ...
        if query and 'IAMP' in query and '>' not in query:
            ...
            with open('IAMP.tsv') as f, open('blast_input.txt', 'w') as g:
                for k in query:
                    f.seek(0,0)
                    l = ' '
                    while(True):
                        i = f.readline()
                        if i=='':
                            lit.error(f'The IAMP Acc. ID {k} does not match with our database. Please re-check')
                            query = None
                            break
                        j = i.split('\t')
                        if k in j[0]:
...
if 'Needleman-Wunsch' in tool:
    ...
    col9, col10 = lit.columns(2)
    with col9:
        query = myquery = lit.text_area('Enter your query protein sequence here:', height=200).upper()
        file_query = lit.file_uploader("Or, you may upload query file")
    with col10:
        subject = mysubject = lit.text_area('Enter your subject protein sequence here:', height=200).upper()
        file_subject = lit.file_uploader("Or, you may upload subject file")
    lit.markdown('<br>', unsafe_allow_html=True)
    gap_open_penalty = 0
    gap_extend_penalty = 0
    lit.write("Parameters:")
    col11, col12 = lit.columns(2)
    with col11:
        try:
    ...
    if query and subject and submit:
        ...
        if 'IAMP' not in query and query.isalpha() is False:
            lit.error("Some non-alphabet is present in the sequence. Please re-check!")
            query = None
        elif 'IAMP' in query and query.isalnum() is False:
            lit.error("Some unrecognized character is present in the Acc. ID. Please re-check!")
            query = None
        if 'IAMP' not in subject and subject.isalpha() is False:
            lit.error("Some non-alphabet is present in the sequence. Please re-check!")
            subject = None
        elif 'IAMP' in subject and subject.isalnum() is False:
            lit.error("Some unrecognized character is present in the Acc. ID. Please re-check!")
            subject = None
        if query and len([i for i in query.split('\n') if i!=''])==1 and 'IAMP' in query:
            with open('IAMP_Non_clinical_all_data.tsv') as f:
                l = ' '
                while(True):
                    i = f.readline()
                    if i=='':
                        lit.error(f'The IAMP Acc. ID {query} does not match with our database. Please re-check')
                        query = None
                        break
                    j = i.split('\t')
                    if query in j[1]:
        ...
        if subject and len([i for i in subject.split('\n') if i!=''])==1 and 'IAMP_' in subject:
            with open('IAMP_Non_clinical_all_data.tsv') as f:
                l = ' '
                while(True):
                    i = f.readline()
                    if i=='':
                        lit.error(f'The IAMP Acc. ID {subject} does not match with our database. Please re-check')
                        subject = None
                        break
                    j = i.split('\t')
                    if subject in j[1]:
        ...
        if query and subject:
            ...
            my_write_file = 'IAMP Needleman-Wunsch Output:\n\nAlignment\n'+myquery+'\n'+mysubject+"\nScore: "+str(score)+'\n'
...
if 'Smith-Waterman' in tool:
    ...
    if query and subject and submit:
        ...
        if 'IAMP' not in query and query.isalpha() is False:
            lit.error("Some non-alphabet is present in the sequence. Please re-check!")
            query = None
        elif 'IAMP' in query and query.replace('_', '').isalnum() is False:
            lit.error("Some unrecognized character is present in the Acc. ID. Please re-check!")
            query = None
        if 'IAMP' not in subject and subject.isalpha() is False:
            lit.error("Some non-alphabet is present in the sequence. Please re-check!")
            subject = None
        elif 'IAMP' in subject and subject.replace('_', '').isalnum() is False:
            lit.error("Some unrecognized character is present in the Acc. ID. Please re-check!")
            subject = None
        if query and len([i for i in query.split('\n') if i!=''])==1 and 'IAMP_' in query:
        ...
        if query and subject:
            ...
            lit.text(mysubject)
            lit.text("Score: "+str(score))
            my_write_file = 'IAMP Smith-Waterman Output:\n\nAlignment\n'+myquery+'\n'+mysubject+"\nScore: "+str(score)+'\n'
